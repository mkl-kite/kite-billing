...
$header = "Кол-во ушедших помесячно";
...
$res = $q->select("
	SELECT 
		year(last_connection) as y,
		month(last_connection) as m,
		IF(LOCATE('{$tags[1]}',p.name)>0,1,IF(LOCATE('{$tags[2]}',p.name)>0,2,0)) as grp,
		count(user) as n
	FROM users u, packets p
	WHERE
		u.pid = p.pid AND u.source!='pigin' AND
		last_connection>DATE_FORMAT(DATE_ADD($now,INTERVAL -14 MONTH),'%Y-%m-01') AND
		last_connection<DATE_FORMAT(DATE_ADD($now,INTERVAL -2 MONTH),'%Y-%m-01')
	GROUP BY y, m, grp
");
$old_m=-1; $i=-1;
foreach($res as $k=>$v) {
